...
static int ConfigSetSocket(DPDKIfaceConfig *iconf, intmax_t entry_int)
{
    ...
    if (entry_int < -1 || entry_int > 4) {
        SCLogError(SC_ERR_INVALID_VALUE,
                "Socket needs to be -1 for any or a valid numa index (usually 0 or 1) %" PRIi64,
                iconf->iface);
        SCReturnInt(-ERANGE);
    }
...
static int ConfigLoad(DPDKIfaceConfig *iconf, const char *iface)
{
    ...
    retval = ConfigSetRxQueues(iconf, 1);//(uint16_t)iconf->threads);
    if (retval < 0)
        SCReturnInt(retval);

    // currently only mapping "1 thread == 1 RX (and 1 TX queue in IPS mode)" is supported
    retval = ConfigSetTxQueues(iconf, 1);//(uint16_t)iconf->threads);
    if (retval < 0)
        SCReturnInt(retval);
        ...
        const struct rte_eth_conf *port_conf)
{
    ...
    iconf->pkt_mempool = rte_pktmbuf_pool_create(mempool_name, iconf->mempool_size,
            iconf->mempool_cache_size, 0, mbuf_size, iconf->socket_id);
    if (iconf->pkt_mempool == NULL) {
        retval = -rte_errno;
        SCLogError(SC_ERR_DPDK_INIT,
