...
static void DPDKReleasePacket(Packet *p)
{
    ...
    if ((p->dpdk_v.copy_mode == DPDK_COPY_MODE_TAP ||
        ...
        retval =
                rte_eth_tx_burst(p->dpdk_v.out_port_id, 0, &p->dpdk_v.mbuf, 1);
        ...
        if (unlikely(retval < 1)) {
            ...
            retval = rte_eth_tx_burst(
                    p->dpdk_v.out_port_id, 0, &p->dpdk_v.mbuf, 1);
            if (unlikely(retval < 1)) {
...
static TmEcode ReceiveDPDKLoop(ThreadVars *tv, void *data, void *slot)
{
    ...
    while (1) {
        ...
        nb_rx = rte_eth_rx_burst(ptv->port_id, 0, ptv->received_mbufs, BURST_SIZE);
        if (unlikely(nb_rx == 0)) {
            continue;
        }
